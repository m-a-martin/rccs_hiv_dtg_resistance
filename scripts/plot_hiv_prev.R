...
pB = ggplot(pred) + 
	...
	geom_errorbar(
		aes(x=round_mid_date, ymin=lwr*100-0.075, ymax=upr*100+0.075, group=label),
	...
	geom_line(
		...
		linewidth=1) + 
	geom_point(
		aes(x=round_mid_date, y=fit*100, group=label, color=label, fill=label),
		size=1.5) +
	xlab('date') + 
...
pC = ggplot(dtg %>% filter(clinic == 'all')) + 
	...
	scale_x_date(limits=c(as.Date(min(d$round_mid_date)), as.Date(max(d$round_max_date)))) + 
