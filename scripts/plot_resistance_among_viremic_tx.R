...
m_pairs = m_pairs %>%
	...
	left_join(years %>% rename(c('round1' = 'round', 'round_mid_date1' = 'round_mid_date')), by='round1') %>%
	left_join(years %>% rename(c('round2' = 'round', 'round_mid_date2' = 'round_mid_date')), by='round2') %>%
	mutate(
		dt = as.numeric((round_mid_date2 - round_mid_date1)/365.25),
